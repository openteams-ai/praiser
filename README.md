...
It scans **GitHub** by default and **Codeberg** (any Gitea/Forgejo host) via
`--forge codeberg`; the pipeline talks to a neutral `Forge` interface, so
adding another host is a self-contained addition.
...
```
praiser <username>
    [--forge github|codeberg]  code host to scan (default: github)
